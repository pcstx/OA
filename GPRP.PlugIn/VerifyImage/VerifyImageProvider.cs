...
namespace GPRP.Plugin.VerifyImage
{
    /// <summary>
    /// 验证码图片创建类
    /// </summary>
            ...
            if (!_instance.ContainsKey(assemlyName))
            {
                lock (lockHelper)
                {
                    if (!_instance.ContainsKey(assemlyName))
                    {
                        ...
                        try
                        {
                            p = (IVerifyImage)Activator.CreateInstance(Type.GetType(string.Format("GPRP.Plugin.VerifyImage.{0}.VerifyImage, GPRP.Plugin.VerifyImage.{0}", assemlyName), false, true));
                        }
                        catch
                        {
                            p = new GPRP.Plugin.VerifyImage.JpegImage.VerifyImage();
                        }
