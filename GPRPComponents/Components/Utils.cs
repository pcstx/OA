...
namespace GPRP.GPRPComponents 
{
    ...
    public class Utils
	{
        ...
        /// <summary>
        /// 清除给定字符串中的回车及换行符,将回车及换行符换成空格
        /// </summary>
        /// <param name="str">要清除的字符串</param>
        /// <returns>清除后返回的字符串</returns>
        public static string ClearEnterAndBR(string str)
        {
            //Regex r = null;
            Match m = null;
            //r = new Regex(@"(\r\n)",RegexOptions.IgnoreCase);
        ...
        /// 返回指定日期格式
        /// </summary>
        public static string GetDate2(string datetimestr, string replacestr)
        {
            if (datetimestr == null)
            {
                return replacestr;
            ...
            try
            {
                datetimestr = Convert.ToDateTime(datetimestr).ToString("yyyy-MM-dd").Replace("2078-12-31", replacestr);
            }
            catch
            {
                return replacestr;
